.class public Landroid/taobao/windvane/view/b;
.super Landroid/taobao/windvane/view/AbstractNaviBar;
.source "WebNaviBar.java"


# instance fields
.field protected a:Landroid/webkit/WebView;

.field private b:I

.field private c:I

.field private d:Landroid/graphics/Paint;

.field private e:Landroid/widget/ImageButton;

.field private f:Landroid/widget/ImageButton;

.field private g:Landroid/widget/ImageButton;

.field private h:[Landroid/graphics/drawable/Drawable;

.field private i:[Landroid/graphics/drawable/Drawable;

.field private j:[Landroid/graphics/drawable/Drawable;


# direct methods
.method public constructor <init>(Landroid/content/Context;Landroid/webkit/WebView;)V
    .locals 3

    .prologue
    const/4 v2, 0x0

    const/4 v1, 0x2

    .line 53
    invoke-direct {p0, p1}, Landroid/taobao/windvane/view/AbstractNaviBar;-><init>(Landroid/content/Context;)V

    .line 28
    const/16 v0, 0x46

    iput v0, p0, Landroid/taobao/windvane/view/b;->b:I

    .line 29
    const/16 v0, 0x64

    iput v0, p0, Landroid/taobao/windvane/view/b;->c:I

    .line 33
    iput-object v2, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    .line 34
    iput-object v2, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    .line 35
    iput-object v2, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    .line 36
    new-array v0, v1, [Landroid/graphics/drawable/Drawable;

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    .line 37
    new-array v0, v1, [Landroid/graphics/drawable/Drawable;

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    .line 38
    new-array v0, v1, [Landroid/graphics/drawable/Drawable;

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    .line 54
    iput-object p2, p0, Landroid/taobao/windvane/view/b;->a:Landroid/webkit/WebView;

    .line 55
    invoke-virtual {p0, p1}, Landroid/taobao/windvane/view/b;->a(Landroid/content/Context;)V

    .line 56
    return-void
.end method

.method static synthetic a(Landroid/taobao/windvane/view/b;)Landroid/widget/ImageButton;
    .locals 1

    .prologue
    .line 20
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    return-object v0
.end method

.method private a(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 5

    .prologue
    const/4 v4, 0x0

    .line 85
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    .line 86
    const/16 v0, 0x70

    .line 87
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    invoke-virtual {v1, v0}, Landroid/widget/ImageButton;->setId(I)V

    .line 88
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    const v1, -0xa0a0b

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 89
    new-instance v0, Landroid/widget/RelativeLayout$LayoutParams;

    iget v1, p0, Landroid/taobao/windvane/view/b;->c:I

    iget v2, p0, Landroid/taobao/windvane/view/b;->c:I

    add-int/lit8 v2, v2, -0x2

    invoke-direct {v0, v1, v2}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    .line 90
    const/16 v1, 0xf

    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 91
    const/16 v1, 0x9

    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 92
    iget v1, p0, Landroid/taobao/windvane/view/b;->b:I

    iput v1, v0, Landroid/widget/RelativeLayout$LayoutParams;->leftMargin:I

    .line 94
    const-string v1, ""

    .line 95
    const-string v2, ""

    .line 96
    iget-object v3, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    invoke-static {p2, v2}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    aput-object v2, v3, v4

    .line 97
    iget-object v2, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    const/4 v3, 0x1

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    aput-object v1, v2, v3

    .line 98
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    iget-object v2, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    aget-object v2, v2, v4

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 99
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    sget-object v2, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 100
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    const/4 v2, 0x2

    invoke-direct {p0, v1, v2}, Landroid/taobao/windvane/view/b;->a(Landroid/widget/ImageButton;I)V

    .line 101
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    invoke-virtual {p0, v1, v0}, Landroid/taobao/windvane/view/b;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    .line 102
    return-void
.end method

.method private a(Landroid/widget/ImageButton;I)V
    .locals 1

    .prologue
    .line 144
    new-instance v0, Landroid/taobao/windvane/view/b$1;

    invoke-direct {v0, p0, p2}, Landroid/taobao/windvane/view/b$1;-><init>(Landroid/taobao/windvane/view/b;I)V

    invoke-virtual {p1, v0}, Landroid/widget/ImageButton;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    .line 164
    return-void
.end method

.method private b(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 5

    .prologue
    const/4 v4, 0x0

    .line 105
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    .line 106
    const/16 v0, 0x71

    .line 107
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    invoke-virtual {v1, v0}, Landroid/widget/ImageButton;->setId(I)V

    .line 108
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    const v1, -0xa0a0b

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 109
    new-instance v0, Landroid/widget/RelativeLayout$LayoutParams;

    iget v1, p0, Landroid/taobao/windvane/view/b;->c:I

    iget v2, p0, Landroid/taobao/windvane/view/b;->c:I

    add-int/lit8 v2, v2, -0x2

    invoke-direct {v0, v1, v2}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    .line 110
    const/16 v1, 0xf

    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 111
    const/4 v1, 0x5

    const/16 v2, 0x70

    invoke-virtual {v0, v1, v2}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(II)V

    .line 112
    iget v1, p0, Landroid/taobao/windvane/view/b;->b:I

    mul-int/lit8 v1, v1, 0x2

    iput v1, v0, Landroid/widget/RelativeLayout$LayoutParams;->leftMargin:I

    .line 113
    const-string v1, ""

    .line 114
    const-string v2, ""

    .line 115
    iget-object v3, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    invoke-static {p2, v2}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    aput-object v2, v3, v4

    .line 116
    iget-object v2, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    const/4 v3, 0x1

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    aput-object v1, v2, v3

    .line 117
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    iget-object v2, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    aget-object v2, v2, v4

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 118
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    sget-object v2, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 119
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    const/4 v2, 0x3

    invoke-direct {p0, v1, v2}, Landroid/taobao/windvane/view/b;->a(Landroid/widget/ImageButton;I)V

    .line 120
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    invoke-virtual {p0, v1, v0}, Landroid/taobao/windvane/view/b;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    .line 121
    return-void
.end method

.method static synthetic b(Landroid/taobao/windvane/view/b;)[Landroid/graphics/drawable/Drawable;
    .locals 1

    .prologue
    .line 20
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    return-object v0
.end method

.method private c(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 5

    .prologue
    const/4 v4, 0x0

    .line 124
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    .line 125
    const/16 v0, 0x72

    .line 126
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    invoke-virtual {v1, v0}, Landroid/widget/ImageButton;->setId(I)V

    .line 127
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    const v1, -0xa0a0b

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 128
    new-instance v0, Landroid/widget/RelativeLayout$LayoutParams;

    iget v1, p0, Landroid/taobao/windvane/view/b;->c:I

    iget v2, p0, Landroid/taobao/windvane/view/b;->c:I

    add-int/lit8 v2, v2, -0x2

    invoke-direct {v0, v1, v2}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    .line 129
    const/16 v1, 0xf

    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 130
    const/16 v1, 0xb

    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 131
    iget v1, p0, Landroid/taobao/windvane/view/b;->b:I

    iput v1, v0, Landroid/widget/RelativeLayout$LayoutParams;->rightMargin:I

    .line 132
    const-string v1, ""

    .line 133
    const-string v2, ""

    .line 134
    iget-object v3, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    invoke-static {p2, v2}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    aput-object v2, v3, v4

    .line 135
    iget-object v2, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    const/4 v3, 0x1

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/e;->a(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    aput-object v1, v2, v3

    .line 137
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    iget-object v2, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    aget-object v2, v2, v4

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 138
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    sget-object v2, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {v1, v2}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 139
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    const/4 v2, 0x4

    invoke-direct {p0, v1, v2}, Landroid/taobao/windvane/view/b;->a(Landroid/widget/ImageButton;I)V

    .line 140
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    invoke-virtual {p0, v1, v0}, Landroid/taobao/windvane/view/b;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    .line 141
    return-void
.end method


# virtual methods
.method public a()V
    .locals 4

    .prologue
    const/4 v3, 0x0

    const/4 v2, 0x1

    .line 167
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    if-eqz v0, :cond_0

    .line 168
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->a:Landroid/webkit/WebView;

    invoke-virtual {v0}, Landroid/webkit/WebView;->canGoForward()Z

    move-result v0

    if-eqz v0, :cond_3

    .line 169
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v1, v2

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 174
    :cond_0
    :goto_0
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    if-eqz v0, :cond_1

    .line 175
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->a:Landroid/webkit/WebView;

    invoke-virtual {v0}, Landroid/webkit/WebView;->canGoBack()Z

    move-result v0

    if-eqz v0, :cond_4

    .line 176
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v1, v2

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 181
    :cond_1
    :goto_1
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    if-eqz v0, :cond_2

    .line 182
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v1, v2

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 184
    :cond_2
    return-void

    .line 171
    :cond_3
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->f:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->i:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v1, v3

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    goto :goto_0

    .line 178
    :cond_4
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->g:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->j:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v1, v3

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    goto :goto_1
.end method

.method protected a(Landroid/content/Context;)V
    .locals 4

    .prologue
    .line 59
    const/16 v0, 0x6e

    invoke-virtual {p0, v0}, Landroid/taobao/windvane/view/b;->setId(I)V

    .line 60
    const v0, -0xa0a0b

    invoke-virtual {p0, v0}, Landroid/taobao/windvane/view/b;->setBackgroundColor(I)V

    .line 61
    const/4 v0, 0x1

    invoke-virtual {p0, v0}, Landroid/taobao/windvane/view/b;->setClickable(Z)V

    .line 64
    invoke-virtual {p1}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    invoke-virtual {v0}, Landroid/content/res/Resources;->getDisplayMetrics()Landroid/util/DisplayMetrics;

    move-result-object v0

    iget v0, v0, Landroid/util/DisplayMetrics;->density:F

    .line 65
    const/high16 v1, 0x42500000    # 52.0f

    mul-float/2addr v1, v0

    const/high16 v2, 0x3f000000    # 0.5f

    add-float/2addr v1, v2

    float-to-int v1, v1

    iput v1, p0, Landroid/taobao/windvane/view/b;->c:I

    .line 66
    new-instance v1, Landroid/view/ViewGroup$LayoutParams;

    const/4 v2, -0x1

    iget v3, p0, Landroid/taobao/windvane/view/b;->c:I

    invoke-direct {v1, v2, v3}, Landroid/view/ViewGroup$LayoutParams;-><init>(II)V

    .line 67
    invoke-virtual {p0, v1}, Landroid/taobao/windvane/view/b;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    .line 69
    new-instance v1, Landroid/graphics/Paint;

    invoke-direct {v1}, Landroid/graphics/Paint;-><init>()V

    iput-object v1, p0, Landroid/taobao/windvane/view/b;->d:Landroid/graphics/Paint;

    .line 70
    iget-object v1, p0, Landroid/taobao/windvane/view/b;->d:Landroid/graphics/Paint;

    const v2, -0x333334

    invoke-virtual {v1, v2}, Landroid/graphics/Paint;->setColor(I)V

    .line 71
    const/high16 v1, 0x41c80000    # 25.0f

    mul-float/2addr v0, v1

    float-to-int v0, v0

    iput v0, p0, Landroid/taobao/windvane/view/b;->b:I

    .line 72
    invoke-virtual {p1}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    .line 76
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/b;->c(Landroid/content/Context;Landroid/content/res/Resources;)V

    .line 78
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/b;->b(Landroid/content/Context;Landroid/content/res/Resources;)V

    .line 80
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/b;->a(Landroid/content/Context;Landroid/content/res/Resources;)V

    .line 82
    return-void
.end method

.method public b()V
    .locals 3

    .prologue
    .line 187
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    if-eqz v0, :cond_0

    .line 188
    iget-object v0, p0, Landroid/taobao/windvane/view/b;->e:Landroid/widget/ImageButton;

    iget-object v1, p0, Landroid/taobao/windvane/view/b;->h:[Landroid/graphics/drawable/Drawable;

    const/4 v2, 0x0

    aget-object v1, v1, v2

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 190
    :cond_0
    return-void
.end method

.method protected onDraw(Landroid/graphics/Canvas;)V
    .locals 6

    .prologue
    const/4 v1, 0x0

    .line 197
    invoke-super {p0, p1}, Landroid/taobao/windvane/view/AbstractNaviBar;->onDraw(Landroid/graphics/Canvas;)V

    .line 198
    invoke-virtual {p0}, Landroid/taobao/windvane/view/b;->getWidth()I

    move-result v0

    int-to-float v3, v0

    iget-object v5, p0, Landroid/taobao/windvane/view/b;->d:Landroid/graphics/Paint;

    move-object v0, p1

    move v2, v1

    move v4, v1

    invoke-virtual/range {v0 .. v5}, Landroid/graphics/Canvas;->drawLine(FFFFLandroid/graphics/Paint;)V

    .line 199
    return-void
.end method
